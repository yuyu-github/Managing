module.exports = {
  'rolevote': async (client, vote, msg, counts, total) => {
  ...
  'kickvote': async (client, vote, msg, counts, total) => {
    const user = await client.users.fetch(vote.user);
    if (user == null) return;
    const member = msg.guild.members.resolve(user);
  ...
  'banvote': async (client, vote, msg, counts, total) => {
    const user = await client.users.fetch(vote.user);
    if (user == null) return;
  ...
  'unbanvote': async (client, vote, msg, counts, total) => {
    const user = await client.users.fetch(vote.user);
    if (user == null) return;
